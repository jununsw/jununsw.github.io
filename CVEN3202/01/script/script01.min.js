...
function makeModel(a, b, tau, beta) {
    ...
    this.getLineLength = function(boxSize) {
        var max = Math.max(Math.abs(this.a), Math.abs(this.b), Math.abs(this.tau));
        var lst = [this.a / max * boxSize, this.b / max * boxSize, this.tau / max * boxSize];
        
        return lst.map(function(ele) {
            return Math.max(Math.abs(ele), 20);
        });
    };
...
function apply_mohr() {
    ...
    $("#mohr-data ul")
        ...
        .after($("<button id='btn-stress' class='btn btn-warning btn-postproc'>Calculate principal stresses</button>"))
